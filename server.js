...
// 🌟 CONFIGURA AQUÍ EL PRECIO ACTUAL DE TU PORCIÓN
// Cada vez que cambies el precio de tu menú, solo actualiza este número:
const PRECIO_PORCION = 40000; // ⬅️ Cambia este valor al precio nuevo (ej: 35000, 32000, etc.)
...
// Inicio de sesión directo y sin complicaciones
...
app.post('/api/pedido', upload.single('comprobante'), (req, res) => {
    try {
        ...
        const total = cantidad * PRECIO_PORCION; // Se calcula automáticamente con el precio actualizado
...
// Actualizar estrictamente el estado del pedido (Pendiente, En camino, Entregado)
app.post('/api/pedidos/:id/estado', (req, res) => {
    try {
        ...
        const idx = pedidos.findIndex(p => String(p.id) === idBuscado);
...
app.delete('/api/pedidos/:id', (req, res) => {
    try {
        ...
        const filtrados = pedidos.filter(p => String(p.id) !== idBuscado);
